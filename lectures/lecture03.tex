\chapter{Jan.~21 --- Minuscule Weights}

\section{Minuscule Weights}
...
\begin{example}
  ...
  are minuscule in this case.
\end{example}

\begin{lemma}
  Every nonzero minuscule weight
  is fundamental.
\end{lemma}

\begin{proof}
  Suppose $\omega$ is minuscule.
...
\begin{proof}
  ...
  $\omega_i$ is minuscule.
\end{proof}

\begin{lemma}\label{lem:minuscule_zero}
  If $\omega \in Q$ with
  $|\langle \omega, \beta \rangle| \le 1$
...
\begin{corollary}
  If $\omega$ is minuscule, then
  $\chi_\omega = \sum_{\gamma \in W \omega} e^\gamma$.
\end{corollary}

\section{Applications of Minuscule Weights}
\begin{prop}
  $\omega \in P_+$
  is minuscule if and only if
